...
所有 UI 元件必須遵守 `DESIGN_SYSTEM.md` 的規範。
**實作任何前端元件前先讀該文件。**

重點摘要：
- 強調色：`var(--accent)` = `#FF5C3A`（珊瑚橙紅）
- 背景：深色系，`--bg-base: #0a0a0a`
- 字型：Geist Sans + Geist Mono
- 禁止 drop shadow，用 border 代替
- 動畫用 Framer Motion，duration ≤ 0.35s
- 平台 icon 單色，不複製品牌原色
...
**Sanitize 規則：** user subprompt 截斷至 300 字，過濾 injection 關鍵字，包在 `<user_instruction>` XML tag 內送給 後端AI Claude。
...
3. user_subprompt 必須 sanitize 後才送給 後端AI Claude
